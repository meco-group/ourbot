...
bool Communicator::addOutgoing(const string& component_name, const string& port_name, int port_nr, const string& remote_address){
    ...
    if (port == NULL){
      log(Error) << "Could not find port " << port_name << " of " << component->getName() << "." << endlog();
      error();
    }
    if (getPort(port_name) == NULL){
      ...
      _connections.push_back(getOutgoingConnection(anti_port, port_nr, remote_address));
    }
    else {
      port->connectTo(getPort(port_name));
    }
    return true;
}
...
bool Communicator::addIncoming(const string& component_name, const string& port_name, int port_nr){
    ...
    if (getPort(port_name) == NULL){
      Port anti_port = port->antiClone();
      port->connectTo(anti_port);
      _connections.push_back(getIncomingConnection(anti_port, port_nr));
    }
    else {
      port->connectTo(getPort(port_name));
    }
...
Connection* Communicator::getIncomingConnection(Port port, int port_nr){
  ...
  if (type.compare("array") == 0)
    return new IncomingConnection <vector<double> > (port, port_nr);
  return NULL;
}


Connection* Communicator::getOutgoingConnection(Port port, int port_nr, const string& remote_address){
  string type = port->getTypeInfo()->getTypeName();
  if (type.compare("char") == 0)
    return new OutgoingConnection <char> (port, port_nr, remote_address);
  if (type.compare("unsigned char") == 0)
    return new OutgoingConnection <unsigned char> (port, port_nr, remote_address);
  if (type.compare("signed char") == 0)
    return new OutgoingConnection <signed char> (port, port_nr, remote_address);
  if (type.compare("int") == 0)
    return new OutgoingConnection <int> (port, port_nr, remote_address);
  if (type.compare("unsigned int") == 0)
    return new OutgoingConnection <unsigned int> (port, port_nr, remote_address);
  if (type.compare("signed int") == 0)
    return new OutgoingConnection <signed int> (port, port_nr, remote_address);
  if (type.compare("short int") == 0)
    return new OutgoingConnection <short int> (port, port_nr, remote_address);
  if (type.compare("unsigned short int") == 0)
    return new OutgoingConnection <unsigned short int> (port, port_nr, remote_address);
  if (type.compare("signed short int") == 0)
    return new OutgoingConnection <signed short int> (port, port_nr, remote_address);
  if (type.compare("long int") == 0)
    return new OutgoingConnection <long int> (port, port_nr, remote_address);
  if (type.compare("signed long int") == 0)
    return new OutgoingConnection <signed long int> (port, port_nr, remote_address);
  if (type.compare("unsigned long int") == 0)
    return new OutgoingConnection <unsigned long int> (port, port_nr, remote_address);
  if (type.compare("float") == 0)
    return new OutgoingConnection <float> (port, port_nr, remote_address);
  if (type.compare("double") == 0)
    return new OutgoingConnection <double> (port, port_nr, remote_address);
  if (type.compare("long double") == 0)
    return new OutgoingConnection <long double> (port, port_nr, remote_address);
  if (type.compare("wchar_t") == 0)
    return new OutgoingConnection <wchar_t> (port, port_nr, remote_address);
  if (type.compare("array") == 0)
    return new OutgoingConnection <vector<double> > (port, port_nr, remote_address);
  return NULL;
}
