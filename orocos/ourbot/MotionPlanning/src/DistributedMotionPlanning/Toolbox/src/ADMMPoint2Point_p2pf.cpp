...
void ADMMPoint2Point::retrieveSharedVariables(map<string, map<string, vector<double>>>& var_dict){
	vector<vector<double>> subst;
	vector<vector<double>> args = {variables, parameters};
	substitutes["fleet_center0"](args, subst);
	variables_admm["x_i"][0] = subst[0][0];
	variables_admm["x_i"][1] = subst[0][1];
	variables_admm["x_i"][2] = subst[0][2];
	variables_admm["x_i"][3] = subst[0][3];
	variables_admm["x_i"][4] = subst[0][4];
	variables_admm["x_i"][5] = subst[0][5];
	variables_admm["x_i"][6] = subst[0][6];
	variables_admm["x_i"][7] = subst[0][7];
	variables_admm["x_i"][8] = subst[0][8];
	variables_admm["x_i"][9] = subst[0][9];
	variables_admm["x_i"][10] = subst[0][10];
	variables_admm["x_i"][11] = subst[0][11];
	variables_admm["x_i"][12] = subst[0][12];
	substitutes["fleet_center1"](args, subst);
	variables_admm["x_i"][13] = subst[0][0];
	variables_admm["x_i"][14] = subst[0][1];
	variables_admm["x_i"][15] = subst[0][2];
	variables_admm["x_i"][16] = subst[0][3];
	variables_admm["x_i"][17] = subst[0][4];
	variables_admm["x_i"][18] = subst[0][5];
	variables_admm["x_i"][19] = subst[0][6];
	variables_admm["x_i"][20] = subst[0][7];
	variables_admm["x_i"][21] = subst[0][8];
	variables_admm["x_i"][22] = subst[0][9];
	variables_admm["x_i"][23] = subst[0][10];
	variables_admm["x_i"][24] = subst[0][11];
	variables_admm["x_i"][25] = subst[0][12];
...
void ADMMPoint2Point::transformSharedSplines(double current_time){
	if(((current_time > 0) and fabs(fmod(round(current_time*1000.)/1000., horizon_time/10)) <1.e-6)){
		...
		vector<double> z_ji_tf(variables_admm["z_ji"]);
		vector<double> l_ji_tf(variables_admm["l_ji"]);
		vector<double> z_ij_tf(variables_admm["z_ij"]);
		vector<double> l_ij_tf(variables_admm["l_ij"]);
		for(int i=0; i<13; i++){
			x_i_tf[0+i] = 0;
			z_i_tf[0+i] = 0;
			l_i_tf[0+i] = 0;
			for(int k=0; k<n_nghb; k++){
				z_ji_tf[0+i+k*n_shared] = 0;
				l_ji_tf[0+i+k*n_shared] = 0;
				z_ij_tf[0+i+k*n_shared] = 0;
				l_ij_tf[0+i+k*n_shared] = 0;
			}
			for(int j=0; j<13; j++){
				x_i_tf[0+i] += splines_tf["xvar_fleet_center0"][i][j]*variables_admm["x_i"][0+j];
				z_i_tf[0+i] += splines_tf["xvar_fleet_center0"][i][j]*variables_admm["z_i"][0+j];
				l_i_tf[0+i] += splines_tf["xvar_fleet_center0"][i][j]*variables_admm["l_i"][0+j];
				for(int k=0; k<n_nghb; k++){
					z_ji_tf[0+i+k*n_shared] += splines_tf["xvar_fleet_center0"][i][j]*variables_admm["z_ji"][0+j+k*n_shared];
					l_ji_tf[0+i+k*n_shared] += splines_tf["xvar_fleet_center0"][i][j]*variables_admm["l_ji"][0+j+k*n_shared];
					z_ij_tf[0+i+k*n_shared] += splines_tf["xvar_fleet_center0"][i][j]*variables_admm["z_ij"][0+j+k*n_shared];
					l_ij_tf[0+i+k*n_shared] += splines_tf["xvar_fleet_center0"][i][j]*variables_admm["l_ij"][0+j+k*n_shared];
				}
			}
		}
		for(int i=0; i<13; i++){
			...
			l_i_tf[13+i] = 0;
			for(int k=0; k<n_nghb; k++){
				z_ji_tf[13+i+k*n_shared] = 0;
				l_ji_tf[13+i+k*n_shared] = 0;
				z_ij_tf[13+i+k*n_shared] = 0;
				l_ij_tf[13+i+k*n_shared] = 0;
			}
			for(int j=0; j<13; j++){
				x_i_tf[13+i] += splines_tf["xvar_fleet_center1"][i][j]*variables_admm["x_i"][13+j];
				z_i_tf[13+i] += splines_tf["xvar_fleet_center1"][i][j]*variables_admm["z_i"][13+j];
				l_i_tf[13+i] += splines_tf["xvar_fleet_center1"][i][j]*variables_admm["l_i"][13+j];
				for(int k=0; k<n_nghb; k++){
					z_ji_tf[13+i+k*n_shared] += splines_tf["xvar_fleet_center1"][i][j]*variables_admm["z_ji"][13+j+k*n_shared];
					l_ji_tf[13+i+k*n_shared] += splines_tf["xvar_fleet_center1"][i][j]*variables_admm["l_ji"][13+j+k*n_shared];
					z_ij_tf[13+i+k*n_shared] += splines_tf["xvar_fleet_center1"][i][j]*variables_admm["z_ij"][13+j+k*n_shared];
					l_ij_tf[13+i+k*n_shared] += splines_tf["xvar_fleet_center1"][i][j]*variables_admm["l_ij"][13+j+k*n_shared];
				}
			}
		}
		variables_admm["x_i"] = x_i_tf;
		variables_admm["z_i"] = z_i_tf;
		variables_admm["l_i"] = l_i_tf;
		variables_admm["z_ji"] = z_ji_tf;
		variables_admm["l_ji"] = l_ji_tf;
		variables_admm["z_ij"] = z_ij_tf;
		variables_admm["l_ij"] = l_ij_tf;
	}
