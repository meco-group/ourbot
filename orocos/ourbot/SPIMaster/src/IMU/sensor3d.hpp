...
class Sensor3D
{
	private:
		...
		int16_t _offset[3];//user-defined offset
		float 	_scale[3];//user-defined scaling factor
	...
	public:
    	...
    	_ID(ID),
			// _raw{0,0,0},
			// _calib(3),  //how initialize at ones/zeros? Todo: this seems wrong, puts everything to zero?
  ...
  //   		_ID(ID),
		...
		std::vector<double> calibrate(){
			for(uint8_t k=0;k<3;k++){
				...
				std::cout<<"offset: "<<(int)_offset[k]<<std::endl;
				std::cout<<"scale: "<<_scale[k]<<std::endl;
				_calib.push_back((_data[k]*_conversion_factor + _offset[k])*_scale[k]);
				std::cout<<"multiplication: "<<(_data[k]*_conversion_factor + _offset[k])*_scale[k]<<std::endl;
				std::cout<<"calibrated data 1: "<<_calib[k]<<std::endl;
			}
			std::cout<<"calibrated data: "<<_calib[0]<<" , "<<_calib[1]<<" , "<<_calib[2]<<std::endl;
			return _calib; 
		}

		void setOffset(int16_t _offset_x, int16_t _offset_y, int16_t _offset_z){
			_offset[0] = _offset_x;
			_offset[1] = _offset_y;
			_offset[2] = _offset_z;
		...
		void setScale(int16_t _scale_x, int16_t _scale_y, int16_t _scale_z){
			_scale[0] = _scale_x;
			_scale[1] = _scale_y;
			_scale[2] = _scale_z;
