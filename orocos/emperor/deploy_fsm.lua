  ...
  deploy = rfsm.state {
    ...
    connect_remote_components = rfsm.state {
      entry = function(fsm)
        ...
        if not addOutgoing('hawkeye', 'robot9_pose_port', 6050, kurt) then rfsm.send_events(fsm, 'e_failed') return end
        ...
        if not addIncoming('hawkeye', 'robot9_est_pose_port', 6000) then rfsm.send_events(fsm,'e_failed') return end
