...
int DifferenceSensor::readRawValue()
{
	int raw_value_new = _sensor->readRawValue();
	uint32_t time = micros();
	
	// caluculate the finite difference
	//_raw_value = ((raw_value_new - _raw_value_old)*1000000)/(time-_last_update);
	_raw_value = (1.0-_alpha)*_raw_value + _alpha*((raw_value_new - _raw_value_old)*1000000)/(time-_last_update);
